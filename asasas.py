...
st.write("Esta aplicación predice la probabilidad de problemas cardiovasculares basado en datos ingresados.")

# Cargar el modelo preentrenado
st.sidebar.header("Configuración")
modelo = joblib.load('modelo_ganador_logreg.pkl')
...
def cargar_preprocesador():
    ...
    ordinalEN = OrdinalEncoder(categories=[
        categorias_Colesterol, categorias_Glucosa, categorias_Fuma, 
        categorias_Toma_alchol, categorias_Actividad_fisica
...
# Predicción
prediccion = modelo.predict(nuevos_datos_transformados)
probabilidad = modelo.predict_proba(nuevos_datos_transformados)[:, 1]

# Mostrar resultados
st.subheader("Resultados de la Predicción")
if prediccion[0] == 1:
    st.write("**El modelo predice que el paciente TIENE riesgo de problemas cardiovasculares.**")
else:
    st.write("**El modelo predice que el paciente NO tiene riesgo de problemas cardiovasculares.**")

st.write(f"Probabilidad estimada de riesgo: **{probabilidad[0]:.2%}**")
st.write("Interpretación: Si la probabilidad supera un umbral clínico específico, se recomienda consultar con un médico.")
