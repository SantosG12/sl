...
# Función para añadir una columna dummy
def add_dummy_column(X):
    """Añadir una columna dummy con valor 0."""
    dummy = np.zeros((X.shape[0], 1))
    return np.hstack([X, dummy])

# Crear el preprocesador con columna dummy
preprocessor = Pipeline([
    ('column_transformer', ColumnTransformer(
        transformers=[
            ("OneHotEncoder", OneHotEncoder(sparse_output=False, drop='first', dtype=np.int64, handle_unknown='ignore'), columnasOHE),
    ...
    ('add_dummy', FunctionTransformer(add_dummy_column))  # Añadir la columna dummy
])
